...
class Trainval(object):
    ...
    def train(self):
        ...
        for i, data in enumerate(tbar):
            ...
            focal, dice, aux_loss, boundary_coh_loss, boundary_cls_loss = self.model(data['img1'].cuda(),
                                                                                     data['img2'].cuda(),
                                                                                     data['cd_label'].cuda(),
                                                                                     data['boundary_mask'].cuda(),
                                                                                     data['boundary_label'].cuda())
            loss = focal * 0.5 + dice + aux_loss * 0.5 + boundary_coh_loss * 0.5 + boundary_cls_loss * 0.5
            ...
            tbar.set_description("Loss: %.3f, Focal: %.3f, Dice: %.3f, Aux: %.3f, Coh: %.3f, Cls: %.3f, LR: %.6f" %
                                 (_loss / (i + 1), _focal_loss / (i + 1), _dice_loss / (i + 1), _aux_loss / (i + 1),
