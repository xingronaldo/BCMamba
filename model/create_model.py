...
class Model(nn.Module):
    ...
    def forward(self, x1, x2, label, boundary_mask, boundary_label):
        ...
        pred, aux_loss, boundary_coh_loss, boundary_cls_loss = self.detector(x1, x2, label, boundary_mask, boundary_label)
        focal = self.focal(pred, label)
        dice = self.dice(pred, label)

        return focal, dice, aux_loss, boundary_coh_loss, boundary_cls_loss
