...
from .block.fusion import Diff, CrossAttn
from .block.decoder import BodyBranch, BoundaryBranch
...
class Detector(nn.Module):
    def __init__(self, channels=128, encoder_depth=2, decoder_depth=2, neigh_size=5, margin=0.5, init_type='normal'):
        ...
        self.diff = Diff(channels)
        self.body = BodyBranch(channels, depth=decoder_depth)
        self.boundary = BoundaryBranch(channels, depth=decoder_depth)
        self.fusion = CrossAttn(channels)

        self.project = nn.Sequential(nn.Conv2d(channels, channels//2, 1, padding=0),
                                     nn.ReLU6())
        self.classifier = nn.Conv2d(channels//2, 2, 1, padding=0)
        self.aux_head = nn.Sequential(nn.Conv2d(channels, channels//2, 1, padding=0),
                                      nn.ReLU6(),
    ...
    def forward(self, x1, x2, label=None, boundary_mask=None, boundary_label=None):
        ...
        ### Mamba Encoder
        fea1 = self.encoder(fea1)
        fea2 = self.encoder(fea2)
        fea = self.diff(fea1, fea2)
        ### Decoders
        body_fea = self.body(fea)
        boundary_fea = self.boundary(fea)
        final_fea = self.fusion(boundary_fea, body_fea)
        pred = self.head(final_fea)

        if label is None:  ### if not is_training
            return pred

        else:  ### if is_training
            label = F.interpolate(label.unsqueeze(1).float(), size=fea.size()[2:], mode='nearest').squeeze(1).long()
            boundary_mask = F.interpolate(boundary_mask.unsqueeze(1).float(), size=fea.size()[2:],
                                          mode='nearest').squeeze(1).long()
            boundary_label = F.interpolate(boundary_label.unsqueeze(1).float(), size=fea.size()[2:],
                                          mode='nearest').squeeze(1).long()

            pred_ = self.aux_head(fea)
            aux_loss = F.cross_entropy(pred_, label)

            boundary_fea_prj = self.project(boundary_fea)
            one_hot_label = torch.eye(2)[label].permute(0, 3, 1, 2).type_as(boundary_fea_prj)
            boundary_coh_loss = self.boundary_coherency_loss(boundary_fea_prj, one_hot_label,
                                                                    boundary_mask, pred)
            pred_boundary = self.classifier(boundary_fea_prj)
            boundary_cls_loss = F.cross_entropy(pred_boundary, boundary_label, ignore_index=255)

            return pred, aux_loss, boundary_coh_loss, boundary_cls_loss
    ...
    def boundary_coherency_loss(self, fea, one_hot_label, boundary_mask, pred):
        ...
        one_hot_pred = torch.eye(2)[pred].permute(0, 3, 1, 2).type_as(fea)
