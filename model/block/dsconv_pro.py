...
class DSCBlock(nn.Module):
    def __init__(self, in_ch, out_ch, kernel_size=9):
        ...
        self.convx = DSConv_pro(in_ch, out_ch, kernel_size, extend_scope=1.0,
                                morph=0, if_offset=True, device="cuda" if torch.cuda.is_available() else "cpu")
        self.convy = DSConv_pro(in_ch, out_ch, kernel_size, extend_scope=1.0,
                                morph=1, if_offset=True, device="cuda" if torch.cuda.is_available() else "cpu")
        self.to_out = Conv(3*out_ch, out_ch)

    def forward(self, x):
        x_conv = self.conv(x)
        x_convx = self.convx(x)
        x_convy = self.convy(x)
        x = torch.cat([x_conv, x_convx, x_convy], dim=1)
        x = self.to_out(x)
        return x